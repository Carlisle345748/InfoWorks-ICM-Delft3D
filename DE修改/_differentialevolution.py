...
class DifferentialEvolutionSolver(object):
    ...
    def __init__(self, func, bounds, start, end, network, run_template, args=(), 
    ...
    def solve(self):
        ...
        for nit in xrange(1, self.maxiter + 1):
            ...
            if self.disp:
                ...
                self.fx_history.append(self.population_energies[0])
                self.x_best_history[nit - 1, :] = self.x
                if nit > 1:
                    np.savetxt('DE_best_x.txt', self.x_best_history)
                    np.savetxt('DE_best_f(x).txt', np.array(self.fx_history))
    ...
    def _calculate_population_energies(self, population):
        ...
        print("ICM开始运行")
        icm = InfoWorks(parameters_pop, self.start, self.end,
                        self.network, self.run_template, plot=False)
